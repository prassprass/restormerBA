...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='E:/S2/Proposal/Restormer-master/data') #Changed path
    parser.add_argument('--data_name', type=str, default='GOPRO', choices=['GOPRO', 'GOPRO_NU', 'HIDE'])
    parser.add_argument('--save_path', type=str, default='E:/S2/Proposal/Restormer-master/result') #Changed path
    ...
    parser.add_argument('--num_iter', type=int, default=50000, help='iterations of training')
...
class BlurDataset(Dataset):
    def __init__(self, data_path, data_name, data_type, patch_size=None, length=None):
        ...
        self.blur_images = sorted(glob.glob('{}/{}/{}/blur_imgs/*.png'.format(data_path, data_name, data_type)))
        self.sharp_images = sorted(glob.glob('{}/{}/{}/sharp_imgs/*.png'.format(data_path, data_name, data_type)))
        # make sure the length of training and testing different
        self.num = len(self.blur_images)
    ...
    def __getitem__(self, idx):
        image_name = os.path.basename(self.blur_images[idx % self.num])
        blur = T.to_tensor(Image.open(self.blur_images[idx % self.num]))
        sharp = T.to_tensor(Image.open(self.sharp_images[idx % self.num]))
        h, w = blur.shape[1:]

        if self.data_type == 'train':
            # make sure the image could be cropped
            blur = pad_image_needed(blur, (self.patch_size, self.patch_size))
            sharp = pad_image_needed(sharp, (self.patch_size, self.patch_size))
            i, j, th, tw = RandomCrop.get_params(blur, (self.patch_size, self.patch_size))
            blur = T.crop(blur, i, j, th, tw)
            sharp = T.crop(sharp, i, j, th, tw)
            if torch.rand(1) < 0.5:
                blur = T.hflip(blur)
                sharp = T.hflip(sharp)
            if torch.rand(1) < 0.5:
            ...
            blur = F.pad(blur, (0, pad_w, 0, pad_h), 'reflect')
            sharp = F.pad(sharp, (0, pad_w, 0, pad_h), 'reflect')
        return blur, sharp, image_name, h, w
