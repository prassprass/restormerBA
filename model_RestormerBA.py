...
class MDTA(nn.Module):
    def __init__(self, channels, num_heads):
        ...
        midplanes = int(self.channels//2)
        
        self.pool_1_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_1_w = nn.AdaptiveAvgPool2d((1, None))
        self.conv_1_h = nn.Conv2d(self.channels, midplanes, kernel_size=(3, 1), padding=(1, 0), bias=False)
        self.conv_1_w = nn.Conv2d(self.channels, midplanes, kernel_size=(1, 3), padding=(0, 1), bias=False)

        self.pool_3_h = nn.AdaptiveAvgPool2d((None, 3))
        self.pool_3_w = nn.AdaptiveAvgPool2d((3, None))
        self.conv_3 = nn.Conv2d(self.channels, midplanes, kernel_size=3, padding=1, bias=False)

        self.pool_5_h = nn.AdaptiveAvgPool2d((None, 5))
        self.pool_5_w = nn.AdaptiveAvgPool2d((5, None))
        self.conv_5 = nn.Conv2d(self.channels, midplanes, kernel_size=3, padding=1, bias=False)

        self.pool_7_h = nn.AdaptiveAvgPool2d((None, 7))
        self.pool_7_w = nn.AdaptiveAvgPool2d((7, None))
        self.conv_7 = nn.Conv2d(self.channels, midplanes, kernel_size=3, padding=1, bias=False)

        self.fuse_conv = nn.Conv2d(midplanes * 4, self.channels, kernel_size=1, padding=0, bias=False)
        self.relu = nn.ReLU(inplace=False)
        self.conv_final = nn.Conv2d(self.channels , self.channels, kernel_size=1, bias=False)
        
        self.mask_conv_1 = nn.Conv2d(self.channels, self.channels, kernel_size=3, padding=1)
        self.mask_relu = nn.ReLU(inplace=False)
        self.mask_conv_2 = nn.Conv2d(self.channels, self.channels, kernel_size=3, padding=1)

        self.temperature = nn.Parameter(torch.ones(1, num_heads, 1, 1))
        self.project_out = nn.Conv2d(channels//3, channels, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        hx = out1 * mask_2
        q, k, v = self.conv_final(hx).chunk(3,dim=1)
